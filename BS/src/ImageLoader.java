...
/*
 * @author Shin Kaya (30 March 2018)  
 */
...
//	Eclipse Wrote: The serializable class ImageLoader does not declare btn static final serialVersionUID field of type long
	...
	// CONSTRUCTORS ============================================================
	public ImageLoader () {						// Constructor #1 
		this(NUM_IMAGE_EMPTY);					// invoke #2
	}
	
	public ImageLoader (int imageNum) {			// Constructor #2 
	...
	public void printAttributes () {
		...
		for (int i = 0; i < ImageLoader.IMAGE_NAME.length; i++) {
			System.out.println(i+ "." + ImageLoader.getImageName(i)) ;
		}
		System.out.println("\n----------------------------------");
	}
	...
	public static ImageIcon getImageIcon(int ImageRequested) {
...
//		logicTestClient ();
	...
	public static void guiTestClient () {
		...
		for (int i = 0; i < 5; i++) {
			btn[i] = new JButton();
			btn[i].setIcon(ImageLoader.getImageIcon(i));
			panel.add(btn[i]);
		}
	...
	public static void logicTestClient () { 
		ImageLoader btn = new ImageLoader();
		System.out.println("\n------------------------------" 
		...
		for (int i = 0; i < IMAGE_NAME.length; i++) {
			try {
				System.out.println(btn.getClass().getResource(ImageLoader.getImageName(i) ));
			} catch (Exception e) {
